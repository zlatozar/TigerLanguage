...
let rec var (expr, d) =
    ...
    | FieldVar(v, s, _)  -> indent d; sayln "FieldVar(";  var (v, d + 1); sayln ","
                            indent (d + 1); say (Store.name s); say ")"

    | SubscriptVar(v, e, _) -> indent d; sayln "SubscriptVar("; var (v, d + 1); sayln ","
                               exp (e, d + 1); say ")"
...
and exp (expr, d) =
    ...
    | StringExp (s, _)  -> indent d; say "StringExp(\""; say s; say "\")"
    ...
    | RecordExp recRec  -> let printFields ((name, e, _), d) =
    ...
    | BreakExp _        -> indent d; say "BreakExp"
...
and ty (typs, d) =
    match typs with
    | NameTy (s, _)           -> indent d; say "NameTy("; say (Store.name s); say ")"
    ...
    | ArrayTy (s, _)          -> indent d; say "ArrayTy("; say (Store.name s); say ")"
