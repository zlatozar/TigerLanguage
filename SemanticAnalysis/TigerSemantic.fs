...
and transDec (venv, fenv, tenv, breakpoint, (dec: Absyn.TDec)) :ProgEnv =
    ...
    | FunctionDec funDecRecList
                         -> printfn "   !!FunctionDec"
                            ...
                            // 2. Then process functions bodies
                            let checkFunDec (funDecRec: FunDecRec) = match Store.lookup (fenv', funDecRec.name) with
                                                                     | None          -> error funDecRec.pos "Tiger Semantic Analysis did not find function header."
